...
sd(hits[hits[,8] < 999,8], na.rm = T) / sqrt(length(hits[hits[,8] < 999,8])-sum(as.numeric(is.na(hits[,8])))))
 ...
 par(mar=c(5,5,2,2))
x <- barplot(msss[c(3,4,5,6,7)], names.arg = c("-2", "-1", "1", "2", "3"), xlab = "Order of entry relative to patch switch", ylab = "Residual Proximity", cex.lab = 1.7, ylim = c(0.20, 0.212), cex.axis = 1.3, cex.names = 1.3, col="grey20")
